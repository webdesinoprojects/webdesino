...
export default async function EmployeeServiceCategoryDetailsPage({ params }: { params: { categoryId: string } }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-end">
        <Link href={`/employee/dashboard/services/${category.id}/new`}>
          <Button>
            ...
            {category.subtypes.map((subtype) => (
              <TableRow key={subtype.id}>
                ...
                <TableCell className="text-right">
                  <ActionsMenu
                    id={subtype.id}
                    editUrl={`/employee/dashboard/services/${category.id}/${subtype.id}`}
