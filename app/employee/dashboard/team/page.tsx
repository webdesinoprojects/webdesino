  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/employee/dashboard/team/new">
            ...
            {members.map((member) => (
              <TableRow key={member.id}>
                ...
                <TableCell className="text-right">
                  <ActionsMenu id={member.id} editUrl={`/employee/dashboard/team/${member.id}`} deleteAction={deleteTeamMember} />
                </TableCell>
              </TableRow>
            ))}
            {members.length === 0 && (
              <TableRow><TableCell colSpan={5} className="text-center py-8 text-slate-500">No team members found.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </div>
