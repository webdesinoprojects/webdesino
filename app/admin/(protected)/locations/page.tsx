...
export default async function LocationsPage({ searchParams }: LocationsPageProps) {
  ...
  const stateDistinctRows = await prisma.locationPage.findMany({ select: { state: true } });
  const stateFilterOptions = mergeStateFilterOptions(
    stateDistinctRows
      .map((r: { state?: string }) => r.state)
      .filter((s: string | undefined): s is string => s != null && s.length > 0)
  );

  const where: Record<string, any> = {};
  if (service) where.serviceFocus = service;
  ...
  const total = await prisma.locationPage.count({ where });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-lg border border-slate-200">
        <p className="text-sm text-slate-600">
          {total === 0 ? (
            ...
            <>
              Showing <span className="font-semibold text-slate-900">{skip + 1}-{Math.min(skip + locations.length, total)}</span>{" "}
              of <span className="font-semibold text-slate-900">{total}</span> location pages
              {stateFilter && (
                <span>
                  {" "}- State: <span className="font-semibold">{labelForLocationState(stateFilter)}</span>
                </span>
              )}
              {service && (
                <span>
                  {" "}- Service: <span className="font-semibold">{services.find((s) => s.value === service)?.label}</span>
                </span>
              )}
              {search && (
                <span>
      ...
      <Suspense fallback={<div className="h-24 rounded-lg border border-slate-200 bg-white animate-pulse" />}>
          ...
          <TableBody>
            {locations.map((loc: any) => (
              <TableRow key={loc.id}>
                ...
                <TableCell className="text-xs text-slate-600 max-w-[10rem]">{labelForLocationState(loc.state)}</TableCell>
                <TableCell className="text-xs">{services.find((s) => s.value === loc.serviceFocus)?.label || loc.serviceFocus}</TableCell>
                <TableCell className="text-xs text-slate-500">{loc.slug}</TableCell>
                <TableCell className="text-right">
                  <ActionsMenu editUrl={`/admin/locations/${loc.id}`} id={loc.id} deleteAction={deleteLocation} itemName="location" />
                </TableCell>
              </TableRow>
            ))}
            {locations.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-10 text-gray-500">
                  No locations found. {service || stateFilter ? "Try adjusting your filters." : "Add one to get started."}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
