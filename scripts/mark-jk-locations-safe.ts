 ...
 * This version checks if state field exists first
 * Run with: npx tsx scripts/mark-jk-locations-safe.ts
 */
...
async function markJKLocations() {
  ...
  try {
    // First, check if state column exists by trying to query it
    try {
      await prisma.$queryRaw`SELECT state FROM "LocationPage" LIMIT 1`;
      console.log("✅ State column exists in database\n");
    } catch (error: any) {
      if (error.code === 'P2022' || error.message.includes('does not exist')) {
        console.log("❌ State column does not exist in database yet!");
    ...
    for (const locationName of JK_LOCATIONS) {
      ...
      // Update all pages for this location using raw SQL to avoid type issues
    ...
    if (notFoundList.length > 0 && notFoundList.length <= 20) {
      ...
      console.log(`\n📋 ${notFoundList.length} locations not found (too many to list)`);
    }
